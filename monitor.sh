...
# =====================================================================
# Configuración Global Base (Asumida del script anterior y nombres de variables)
# =====================================================================
UMBRAL_DISCO=90
UMBRAL_RAM=85
UMBRAL_CPU=80
INTERVALO=60
LOG_BASE="/var/log/monitor_recursos"
MAX_LOGS=7
TMP_DIR="/tmp"
LOG_DIR="/var/log"
ENV_FILE=".env"

# Estados iniciales para evitar spam de alertas
ESTADO_DISCO="OK"
ESTADO_RAM="OK"
ESTADO_CPU="OK"
...
enviar_telegram() {
    ...
    if ! curl -s --max-time 10 -X POST "$url" \
        ...
        log "❌ Falló enviando alerta a Telegram."
        return 1
    fi
}
...
limpiar_tmp() {
    ...
    log "Limpieza de /tmp: ${count} archivo(s) eliminado(s)."
}
...
revisar_ram() {
    local USO procesos mensaje linea
